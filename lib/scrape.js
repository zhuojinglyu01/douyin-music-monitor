...
// 搜一个话题词，截获搜索接口（新 /search/item/ 或老 /general/search/single/）。只留命中词、且近 recentDays 天内发布的视频。
export async function scrapeHashtag(page, term, { timeoutMs = 25000, debug = false, recentDays = 0, keywords = [] } = {}) {
  const byId = new Map();
  let apiHit = false;
  const now = Date.now();
  const maxAgeMs = recentDays > 0 ? recentDays * 86400000 : Infinity;
  // 命中判断：标签 或 文案 命中任一命中词即可（双路）；至少拿搜索词兜底
  const words = keywords.length ? keywords : [term];

  const onResp = async (resp) => {
    // 抖音 2026 起把视频搜索接口从 /general/search/single/ 改成了 /search/item/。两个都认。
    if (!/\/(general\/search\/single|search\/item)\//.test(resp.url())) return;
    try {
      const json = await resp.json();
      const data = json?.data || json?.aweme_list || [];
      let got = 0;
      for (const d of data) {
        const a = d?.aweme_info || (d?.aweme_id ? d : null);
        if (!a?.aweme_id) continue;
        // 话题来源：text_extra.hashtag_name + cha_list.cha_name（两处合并去重）
        const tags = [
          ...(a.text_extra || []).map((t) => t.hashtag_name),
          ...(a.cha_list || []).map((c) => c.cha_name),
        ].filter(Boolean);
        const desc = a.desc || "";
        // 双路命中：标签含该词 或 文案含该词
        const hits = words.filter((w) => tags.some((t) => t.includes(w)) || desc.includes(w));
        ...
        byId.set(a.aweme_id, {
          ...
          followers: a?.author?.follower_count ?? null,
          tags,
          hit: [...new Set(hits)], // 命中的词,推送里用
          url: `https://www.douyin.com/video/${a.aweme_id}`,
        });
        got++;
      }
    ...
    try {
      ...
      if (d?.aweme_id === awemeId && d?.statistics?.digg_count != null) {
        result = {
          ...
          followers: d?.author?.follower_count ?? null,
        };
      }
    } catch {
      /* 非 JSON */
